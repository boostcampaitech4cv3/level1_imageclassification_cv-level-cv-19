...
import pandas as pd
class StratifiedProfileDataset(MaskBaseDataset):
    """
    ...
    def _split_profile(profiles, val_ratio):
        ...
        df = pd.read_csv("/opt/ml/input/data/train/train_data.csv")
        class_list = df[["profile"]].values.tolist()
        ...
        for idx, profile in enumerate(class_list):
            profile = profile[0]
            if profile == 612:
                man_young.append(idx)
            elif profile == 1713:
                man_normal.append(idx)
            elif profile == 2814:
                man_old.append(idx)
            elif profile == 3915:
                female_young.append(idx)
            elif profile == 41016:
                female_normal.append(idx)
            elif profile == 51117:
                female_normal.append(idx)
        ...
        for class_type in [man_young, man_normal, man_old, female_young, female_normal, female_old]:
                #val_indices.update(set(random.sample(class_type, k=n_val)))
                tmp = pd.Series(class_type)
                tmp = tmp.sample(frac = val_ratio)
                tmp = tmp.tolist()
                val_indices.update(set(tmp))
...
a = StratifiedProfileDataset("/opt/ml/input/data/train/images")
...
class LabelSplitByProfileDataset(LabelSplitDataset):
    """
        train / val 나누는 기준을 이미지에 대해서 random 이 아닌
        사람(profile)을 기준으로 나눕니다.
        구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다
